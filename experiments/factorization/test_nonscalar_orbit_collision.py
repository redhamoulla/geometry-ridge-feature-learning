...
def build_row(kind: str, pair_id: int, q: np.ndarray) -> dict:
    ...
    row = {
        "pair_id": pair_id, "kind": kind, "label": 0 if kind == "A" else 1,
        "trace_contraction": trace_c, "information_gain": info,
        "displacement_norm": norm_d, "cook_post_norm_sq": cook_post,
...
def grouped_auc(df: pd.DataFrame, columns: list[str]) -> float:
    X = np.round(df[columns].to_numpy(float), 10)
...
def main() -> None:
    ...
    scalar_cols = ["trace_contraction", "information_gain", "displacement_norm", "cook_post_norm_sq", "target_effect"]
    orbit_cols = ["contraction_eigenvalue_1", "contraction_eigenvalue_2", "contraction_eigenvalue_3", "effort_energy_1", "effort_energy_2", "effort_energy_3"]
    aucs = {
        "matched_scalar_summaries": grouped_auc(df, scalar_cols),
        "contraction_spectrum_only": grouped_auc(df, orbit_cols[:3]),
        "effort_distribution_only": grouped_auc(df, orbit_cols[3:]),
        "full_orbit_signature": grouped_auc(df, orbit_cols),
    }
    pair_diff = df.pivot(index="pair_id", columns="kind", values=scalar_cols)
    max_scalar_diff = {col: float(np.max(np.abs(pair_diff[(col, "A")] - pair_diff[(col, "B")]))) for col in scalar_cols}
    summary = {
        "n_pairs": n_pairs,
    ...
    axes[0].plot(x, vals_a / scale, marker="o", label="register A")
    axes[0].plot(x, vals_b / scale, marker="s", linestyle="--", label="register B")
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(["trace", "info", "$\\|d\\|$", "Cook", "target"], rotation=20)
    axes[0].set_ylabel("Normalized value")
    axes[0].set_title("Exactly matched scalarizations")
    axes[0].legend(frameon=False, fontsize=8)
    axes[0].grid(alpha=0.2)
    k = np.arange(1, 4)
    axes[1].plot(k, np.sort(C_A)[::-1], marker="o", label="spectrum A")
    axes[1].plot(k, np.sort(C_B)[::-1], marker="s", linestyle="--", label="spectrum B")
    axes[1].set_xticks(k)
    axes[1].set_xlabel("Eigen-direction")
    axes[1].set_ylabel("Contraction eigenvalue")
    axes[1].set_title("The non-scalar register separates them")
