#!/usr/bin/env python3
"""Functional, parameterization-aware extension of the nonlinear register experiment.
...
def hidden_features(theta: np.ndarray, X: np.ndarray, d: int, h: int) -> np.ndarray:
    W, b, _, _ = unpack(theta, d, h)
    Z = X @ W.T + b
    return np.maximum(Z, 0.0)


def normalized_alignment(A: np.ndarray, B: np.ndarray) -> float:
    na = float(np.linalg.norm(A, ord="fro")); nb = float(np.linalg.norm(B, ord="fro"))
    if na <= 1e-14 or nb <= 1e-14:
        return float("nan")
    return float(np.sum(A * B) / (na * nb))


def normalized_drift(A: np.ndarray, B: np.ndarray) -> float:
    na = float(np.linalg.norm(A, ord="fro")); nb = float(np.linalg.norm(B, ord="fro"))
    if na <= 1e-14 or nb <= 1e-14:
        return float("nan")
    return float(np.linalg.norm(A / na - B / nb, ord="fro"))
...
def train_one(seed: int, cfg: Config) -> Tuple[pd.DataFrame, dict]:
    ...
    # Fixed clean probe set used only to compare functional tangent operators.
    functional_ids = rng.choice(np.arange(len(X_pool)), size=min(140, len(X_pool)), replace=False)
    X_functional = X_pool[functional_ids].copy()
    ...
    checkpoint_kernel: Dict[int, np.ndarray] = {}
    checkpoint_repr_gram: Dict[int, np.ndarray] = {}
    ...
    for epoch in range(cfg.epochs + 1):
        if epoch in cfg.checkpoints:
            ...
            _, J_func = forward_and_jacobian(theta, X_functional, cfg.n_features, cfg.hidden)
            H_func = hidden_features(theta, X_functional, cfg.n_features, cfg.hidden)
            checkpoint_kernel[epoch] = (J_func @ J_func.T) / max(J_func.shape[1], 1)
            checkpoint_repr_gram[epoch] = (H_func @ H_func.T) / max(H_func.shape[1], 1)
            with torch.no_grad():
    ...
    K0 = checkpoint_kernel[e0]
    H0 = checkpoint_repr_gram[e0]
    meta = {
        "seed": seed,
        ...
        "kernel_alignment_initial_final": normalized_alignment(K0, checkpoint_kernel[ef]),
        "kernel_normalized_drift_initial_final": normalized_drift(K0, checkpoint_kernel[ef]),
        "representation_alignment_initial_final": normalized_alignment(H0, checkpoint_repr_gram[ef]),
        "representation_normalized_drift_initial_final": normalized_drift(H0, checkpoint_repr_gram[ef]),
    }
    for epoch in cfg.checkpoints:
        meta[f"kernel_alignment_epoch_{epoch}"] = normalized_alignment(K0, checkpoint_kernel[epoch])
        meta[f"representation_alignment_epoch_{epoch}"] = normalized_alignment(H0, checkpoint_repr_gram[epoch])
    return df, meta
...
def aggregate_and_plot(all_df: pd.DataFrame, meta_df: pd.DataFrame, out_dir: Path, cfg: Config) -> dict:
    ...
    meta_df.to_csv(out_dir / "nonlinear_functional_drift_replicates.csv", index=False)
    ...
    # Functional-operator dynamics under the declared background metric g0=I.
    align_rows = []
    for _, row in meta_df.iterrows():
        for epoch in cfg.checkpoints:
            align_rows.append({
                "seed": int(row["seed"]),
                "epoch": epoch,
                "kernel_alignment": float(row[f"kernel_alignment_epoch_{epoch}"]),
                "representation_alignment": float(row[f"representation_alignment_epoch_{epoch}"]),
            })
    align_df = pd.DataFrame(align_rows)
    align_df.to_csv(out_dir / "nonlinear_functional_drift_dynamics.csv", index=False)
    fig, ax = plt.subplots(figsize=(6.6, 4.2))
    for col, label in [("kernel_alignment", "$K_t^{(0)}$"), ("representation_alignment", "Gram des représentations")]:
        agg = align_df.groupby("epoch")[col].agg(["mean", "std"]).reset_index()
        ax.plot(agg["epoch"], agg["mean"], marker="o", label=label)
        ax.fill_between(agg["epoch"], agg["mean"]-agg["std"], agg["mean"]+agg["std"], alpha=0.15)
    ax.set_xlabel("Époque")
    ax.set_ylabel("Alignement avec l'initialisation")
    ax.set_ylim(0.0, 1.03)
    ax.set_title("Évolution fonctionnelle sous une métrique de fond fixée")
    ax.grid(alpha=0.2)
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(out_dir / "nonlinear_functional_drift.pdf")
    plt.close(fig)
    ...
    result = {
        ...
        "replicate_dynamics": {
            ...
            "kernel_alignment_initial_final_mean": float(meta_df["kernel_alignment_initial_final"].mean()),
            "kernel_alignment_initial_final_std": float(meta_df["kernel_alignment_initial_final"].std(ddof=1)),
            "kernel_normalized_drift_initial_final_mean": float(meta_df["kernel_normalized_drift_initial_final"].mean()),
            "representation_alignment_initial_final_mean": float(meta_df["representation_alignment_initial_final"].mean()),
            "representation_alignment_initial_final_std": float(meta_df["representation_alignment_initial_final"].std(ddof=1)),
            "representation_normalized_drift_initial_final_mean": float(meta_df["representation_normalized_drift_initial_final"].mean()),
        },
    }
    with open(out_dir / "nonlinear_functional_drift_summary.json", "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    for r in range(cfg.replicates):
        ...
        print(f"replicate {r+1}/{cfg.replicates}: mse={meta['test_mse_final']:.4f}, K-align={meta['kernel_alignment_initial_final']:.3f}")
